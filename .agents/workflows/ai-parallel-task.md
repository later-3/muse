...
> **适用范围：** `user/research/dayXX/` 下的 AI 并行产出（🧪 实验 + 🔧 消险）
 ...
 *
 * 🔧 运行方式:
 *   node --test user/research/dayXX/expXX-*.test.mjs
 *
 * 💡 设计选择:
 ...
 * expXX 测试 — [一句话]
 *
 * 运行: node --test user/research/dayXX/expXX-*.test.mjs
 *
 * 测试策略:
 * - 基础功能: 验证模式本身正确
...
```bash
# 跑单个实验
node --test user/research/day01/exp01-chain-parallel-route.test.mjs

# 跑全部实验
node --test user/research/day0{1,2,3,4,5,6,7}/*.test.mjs

# 只看结果，不看过程
node --test user/research/day0{1,2,3,4,5,6,7}/*.test.mjs 2>&1 | grep -E '(✔|✖|tests|pass|fail)'
```
