...
| driver | workflow-prompt 注入给 active Muse 的指令 |
|--------|------------------------------------------|
| `"self"`（T39 默认） | "完成后**立即调用 workflow_transition** 推进" |
| `"planner"` | "完成后**通知 Planner 你完成了**，不要自己调 workflow_transition" |

```javascript
// workflow-prompt.mjs 改动（伪代码）
const isPlanner = sm.definition.driver === 'planner'
...
if (node.wait_for_user) {
  ...
  // T39 原逻辑: "完成后立即调用 workflow_transition"
}
```
...
| **Planner** | — | ✅ 通过 `workflow_admin_transition` 调 |
| **用户** | ✅ /wf approve（actor=user） | ✅ 通过 Planner 中转 |

**实现方式**：不改 `handleWorkflowTransition`，新增 `workflow_admin_transition` MCP 工具：

```javascript
// planner-tools.mjs
export async function handleAdminTransition(args) {
  const { instance_id, event, reason } = args
  // 1. 按 instance_id 找 SM（不依赖 session 绑定，是 admin 级权限）
  const sm = registry.getByInstanceId(instance_id)
  // 2. 以 'admin' actor 执行 transition
...
| 机制 | 归属 | 语义 |
|------|------|------|
| `wait_for_user` | T39 现有 | 节点级标记，workflow-prompt 注入"等待用户指示"给 **active Muse** |
| user_review | Planner 逻辑 | **Planner 自己决定**何时向用户请示，基于：节点类型/产出重要性/工作流设计 |

**不冲突**：
- `wait_for_user` 在 `driver=planner` 模式下**依然有效**，但语义变为：Planner 在该节点向用户请示（不是 active Muse 直接和用户交互）
- Planner 可以在任何节点决定请用户审核，不需要 JSON schema 标记
...
| `knowledge/quality-checklist.md` | 新增 | 产出质量检查标准 |
| `knowledge/safety-rules.md` | 新增 | 安全红线规则 |
...
| `state-machine.test.mjs` | rollback 到已访问节点 ✅ / 未访问节点 ❌ / history 追加 / actor 只能 system/admin |
| `registry.test.mjs` | getByInstanceId 正确返回 / 不存在返回 null |
| `workflow-prompt.test.mjs` | driver=self → "调 transition" / driver=planner → "通知 planner" |
...
| # | 场景 | 验证点 |
|---|------|--------|
| 1 | pua 写文档 → Planner 检查 → 用户通过 | 完整 2 节点跑通 |
| 2 | 用户要求修改 → 转发 → 返工 | 反馈闭环 |
