...
新建 `src/mcp/planner-tools.mjs`，实现 Planner 专属的 6 个 MCP 工具，并在 `src/mcp/memory.mjs` 中按 role 注册。
...
| 4.7 | 工具注册 | memory.mjs 按 role 条件注册 |
...
export async function handleWorkflowInspect(sessionId, args) {
  ...
  // state.current_node = 当前节点 ID（不是 state.current）
  // sm.definition.listNodes() 返回 [{id, type, participant, ...}]
...
export async function handleWorkflowRollback(sessionId, args) {
  ...
  try {
    // sm.rollback(targetNodeId, actor, reason, meta) — 签名来自 state-machine.mjs:281
...
```javascript
import { executeHandoff, buildHandoffPrompt } from '../family/handoff.mjs'
...
export async function handleHandoffToMember(sessionId, args) {
  ...
  // 使用真实 API：executeHandoff({sm, nextNode, instanceId, sourceRole, client})
  // executeHandoff 来自 src/family/handoff.mjs:43
  // MemberClient 来自 src/family/member-client.mjs:18
  try {
    // 1. 创建目标成员的 MemberClient
    //    需要从 family registry 解析 member 的 host:port
    const client = new MemberClient(/* targetMemberConfig */)
...
| 模块 | 路径 | 用途 |
|------|------|------|
| `executeHandoff()` | `src/family/handoff.mjs:43` | 3-step ACK 协议（PREPARE → BIND+ACK → EXECUTE） |
...
真实的 MCP 注册入口是 `src/mcp/memory.mjs`（不是 server.mjs，仓库里没有 server.mjs）：

- **L858** `ListToolsRequestSchema` — 返回工具列表：
  ```javascript
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [...TOOLS, ...GOAL_TOOLS, ...THREAD_TOOLS, ...DEV_TOOLS, ...TELEGRAM_TOOLS, ...IMAGE_TOOLS, ...WORKFLOW_TOOLS],
  }))
  ```
- **L863** `CallToolRequestSchema` — switch 分发到各 handler
...
```javascript
import { PLANNER_TOOLS, handleWorkflowCreate, handleWorkflowAdminTransition,
  handleWorkflowInspect, handleWorkflowRollback, handleHandoffToMember,
  handleReadArtifact } from './planner-tools.mjs'
```
...
Planner 工具只在 `MUSE_ROLE=planner` 时注册。执行者看不到这些工具名，也无法调用。
...
4. **rollback 的签名** — `sm.rollback(targetNodeId, actor, reason, meta)` 四个参数，不是 `sm.rollback(target, { reason })`
5. **路径安全** — `read_artifact` 必须防止 `../` 路径穿越
6. **handoff 依赖** — `handoff_to_member` 必须使用 `executeHandoff()` + `MemberClient`，不要重新实现。`triggerHandoff` 是 `workflow-tools.mjs` 的私有函数，不可导入
7. **注册入口** — 在 `src/mcp/memory.mjs`（L858/L863），不是 server.mjs（不存在）
...
| 17 | memory.mjs: MUSE_ROLE=planner → 注册 Planner 工具 | grep 检查 |
| 18 | memory.mjs: MUSE_ROLE!=planner → 不注册 Planner 工具 | grep 检查 |
