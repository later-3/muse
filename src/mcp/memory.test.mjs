...
describe('Memory MCP — get_user_profile', () => {
  ...
  it('should always return complete skeleton even if empty', () => {
    ...
    assert.deepEqual(data.structured_goals, [])

    emptyMemory.stop()
    rmSync(dir2, { recursive: true, force: true })
  })

  it('should include threads when threads module provided', () => {
    const threads = new Threads(memory.getDb(), 'muse')
    threads.init()
    threads.create({ title: '健康-跑步', category: 'health' })
    threads.create({ title: '学 Rust', category: 'learning' })

    const result = handleGetUserProfile(memory, {}, { threads })
    const data = getText(result)
    assert.equal(data.important_threads.length, 2)
    // list() returns threads, verify both exist
    const titles = data.important_threads.map(t => t.title)
    assert.ok(titles.includes('健康-跑步'))
    assert.ok(titles.includes('学 Rust'))
    assert.ok('episode_count' in data.important_threads[0])
    assert.ok('last_activity' in data.important_threads[0])
  })

  it('should return empty threads when no threads module', () => {
    const result = handleGetUserProfile(memory, {})
    const data = getText(result)
    assert.deepEqual(data.important_threads, [])
    assert.deepEqual(data.structured_goals, [])
  ...
  it('should include structured goals when goals module provided', () => {
    const goalsModule = new Goals(memory.getDb(), 'muse')
    goalsModule.init()
    goalsModule.create({ title: '学 Rust', category: 'learning' })

    const result = handleGetUserProfile(memory, {}, { goals: goalsModule })
    const data = getText(result)
    assert.equal(data.structured_goals.length, 1)
    assert.equal(data.structured_goals[0].title, '学 Rust')
    assert.ok('progress' in data.structured_goals[0])
    assert.ok('is_overdue' in data.structured_goals[0])
  })
})
...
// T35: Goal MCP Tool Tests
describe('Memory MCP — create_goal', () => {
  let memory, goals, dir

  before(async () => {
    ({ memory, dir } = createTestMemory())
    await memory.start()
    goals = new Goals(memory.getDb(), 'muse')
    goals.init()
  })
  after(async () => {
    await memory.stop()
  ...
  it('should create goal via MCP handler', () => {
    const result = handleCreateGoal(goals, { title: '学 Rust', category: 'learning' })
    const data = getText(result)
    assert.ok(data.id.startsWith('goal-'))
    assert.equal(data.title, '学 Rust')
    assert.equal(data.status, 'active')
    assert.equal(data.category, 'learning')
  })

  it('should return error for missing title', () => {
    const result = handleCreateGoal(goals, {})
    assert.equal(result.isError, true)
  })

  it('should accept agent:xxx source (cross-agent extensibility)', () => {
    const result = handleCreateGoal(goals, { title: '冥想', source: 'agent:muse-a' })
    const data = getText(result)
    assert.equal(data.source, 'agent:muse-a')
  })
})

describe('Memory MCP — list_goals', () => {
  let memory, goals, dir
  ...
  before(async () => {
    ...
    goals = new Goals(memory.getDb(), 'muse')
    goals.init()
    goals.create({ title: 'A', category: 'work' })
    goals.create({ title: 'B', category: 'health' })
    goals.achieve(goals.create({ title: 'C' }).id)
  })
  after(async () => {
    await memory.stop()
    rmSync(dir, { recursive: true, force: true })
  ...
  it('should list all goals', () => {
    const result = handleListGoals(goals, {})
    const data = getText(result)
    assert.equal(data.count, 3)
  })

  it('should filter by status', () => {
    const result = handleListGoals(goals, { status: 'active' })
    const data = getText(result)
    assert.equal(data.count, 2)
  })

  it('should filter by category', () => {
    const result = handleListGoals(goals, { category: 'health' })
    const data = getText(result)
    assert.equal(data.count, 1)
    assert.equal(data.goals[0].title, 'B')
  })
})

describe('Memory MCP — update_goal', () => {
  let memory, goals, dir, goalId
  ...
  before(async () => {
    ...
    goals = new Goals(memory.getDb(), 'muse')
    goals.init()
    goalId = goals.create({ title: '减肥' }).id
  })
  after(async () => {
    await memory.stop()
    rmSync(dir, { recursive: true, force: true })
  ...
  it('should update progress via MCP handler', () => {
    const result = handleUpdateGoal(goals, { id: goalId, progress: 50, note: '减了5斤' })
    const data = getText(result)
    assert.equal(data.progress, 50)
    assert.equal(data.notes.length, 1)
  })

  it('should achieve via MCP handler', () => {
    const newId = goals.create({ title: '完成' }).id
    const result = handleUpdateGoal(goals, { id: newId, status: 'achieved', note: '搞定了' })
    const data = getText(result)
    assert.equal(data.status, 'achieved')
  })

  it('should return error for missing id', () => {
    const result = handleUpdateGoal(goals, {})
    assert.equal(result.isError, true)
  })

  it('should return error for nonexistent id', () => {
    const result = handleUpdateGoal(goals, { id: 'nonexistent', progress: 10 })
    assert.equal(result.isError, true)
  })
})

// T36: Thread MCP Tool Tests
describe('Memory MCP — create_thread', () => {
  let memory, threads, dir

  before(async () => {
    ({ memory, dir } = createTestMemory())
    await memory.start()
    threads = new Threads(memory.getDb(), 'muse')
    threads.init()
  })
  after(async () => {
    await memory.stop()
  ...
  it('should create thread via MCP handler', () => {
    const result = handleCreateThread(threads, { title: '健康-跑步', category: 'health' })
    const data = getText(result)
    assert.ok(data.id.startsWith('thread-'))
    assert.equal(data.title, '健康-跑步')
    assert.equal(data.category, 'health')
  })

  it('should return error for missing title', () => {
    const result = handleCreateThread(threads, {})
    assert.equal(result.isError, true)
  })

  it('should accept goalId association', () => {
    const result = handleCreateThread(threads, { title: '学 Rust', goalId: 'goal-123' })
    const data = getText(result)
    assert.equal(data.goal_id, 'goal-123')
  })
})

describe('Memory MCP — list_threads', () => {
  let memory, threads, dir
  ...
  it('should list all threads', () => {
    const result = handleListThreads(threads, {})
    const data = getText(result)
    assert.equal(data.count, 2)
  })

  it('should filter by category', () => {
    const result = handleListThreads(threads, { category: 'health' })
    const data = getText(result)
    assert.equal(data.count, 1)
    assert.equal(data.threads[0].title, '健康-跑步')
  })
})

describe('Memory MCP — get_thread', () => {
  let memory, threads, dir, threadId
  ...
  before(async () => {
    ...
    threads = new Threads(memory.getDb(), 'muse')
    threads.init()
    threadId = threads.create({ title: '测试主题' }).id
  })
  after(async () => {
    await memory.stop()
    rmSync(dir, { recursive: true, force: true })
  ...
  it('should return thread with episodes', () => {
    const result = handleGetThread(threads, { id: threadId })
    const data = getText(result)
    assert.equal(data.thread.title, '测试主题')
    assert.ok(Array.isArray(data.episodes))
  })

  it('should return error for missing id', () => {
    const result = handleGetThread(threads, {})
    assert.equal(result.isError, true)
  })

  it('should return error for nonexistent', () => {
    const result = handleGetThread(threads, { id: 'nonexistent' })
    assert.equal(result.isError, true)
  })
})
