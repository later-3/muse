 ...
 * Run with: cd muse && node --test mcp/memory.e2e.mjs
 */
...
describe('Memory MCP — E2E (opencode integration)', { skip: !checkOpencode() && 'opencode not available' }, () => {
  ...
  describe('AI autonomous set_memory', () => {
    it('should call set_memory when user states personal info', { timeout: 60_000 }, () => {
      ...
      const dbCheck = run(
        `sqlite3 ./data/memory.db "SELECT value FROM semantic_memory WHERE key LIKE '%e2e%test%user%42%' OR value LIKE '%E2E-Test-User-42%' LIMIT 1;"`,
        { allowFail: true }
      )
  ...
  describe('Audit trail persistence', () => {
    it('should have audit records for the set_memory calls', () => {
      const audits = run(
        `sqlite3 ./data/memory.db "SELECT COUNT(*) FROM memory_audit WHERE new_value LIKE '%E2E-Test-User-42%';"`,
        { allowFail: true }
      )
  ...
  describe('Overwrite guard E2E', () => {
    it('should block ai_inferred from overriding user_stated in real flow', async () => {
      ...
      const config = { memory: { dbPath: './data/memory.db', maxEpisodicDays: 7 } }
  ...
  after(() => {
    ...
    try {
      run(`sqlite3 ./data/memory.db "DELETE FROM semantic_memory WHERE key LIKE '%e2e%' OR value LIKE '%E2E-Test-User-42%';"`)
      run(`sqlite3 ./data/memory.db "DELETE FROM memory_audit WHERE new_value LIKE '%E2E-Test-User-42%' OR target_key LIKE '%e2e%';"`)    } catch {
      // Best effort cleanup
    }
  })
})
