...
> **总目标 A：** 完成 Muse 的 3+1 个场景 MVP（S1 日常对话 / S2 任务协作 / S3 审批治理 / S2b 自开发闭环）  
...
| 场景 | 一句话 | 验收标准 | 实现于 |
|------|-------|---------|--------|
| **S1** 日常对话 | Later 和 pua 聊天，pua 记住过去、有性格、主动问候 | 多轮对话 + 跨 session 记忆 + 人格一致 | Sprint 6 |
| **S2** 任务协作 | Later 让 planner 安排 arch/coder 完成代码任务 | Handoff 成功 + Worker 独立执行 + 结果回传 | Sprint 6 |
| **S3** 审批治理 | 高风险动作需要 Later 审批 | 拦截 → 审批请求 → 批准/拒绝 → 执行/阻止 | Sprint 7 |
| **S2b** 自开发闭环 | Muse 围绕自身问题发起修复链路 | 手机 Telegram 指挥 Muse 干活 | Sprint 7 |

---

## Muse Basic v1 验收（Sprint 7 退出条件）

完成后应有 **5 项能力**：

1. ✅ **S1 会话与记忆** — 多轮对话，跨 session 记忆，人格一致
2. ✅ **S2 任务协作** — planner 拆任务，worker 执行回传，失败重试
3. ✅ **S3 审批治理** — 高风险拦截，审批，超时处理
4. ✅ **S2b 自开发闭环** — Muse 能围绕自身问题发起修复链路
5. ✅ **最小可观测性** — 关键链路 trace，成功率/延迟/错误可查
