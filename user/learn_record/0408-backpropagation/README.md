...
Step 2 算损失：Loss = (6 - 10)^2 = (-4)^2 = 16
Step 3 算梯度：
  Loss 对 w 的梯度怎么算？链式法则，一步步来：
  - Loss = (y - 10)^2   → Loss 对 y 的变化率 = 2*(y - 10) = 2*(6-10) = -8
  - y = w * x           → y 对 w 的变化率 = x = 2
  - 链式法则相乘：Loss 对 w 的梯度 = -8 * 2 = -16

Step 4 更新：w = 3 - 0.01 * (-16) = 3.16  ← w 变大了（因为 y=6 太小了，需要增大 w）
```
...
**先解释一个符号：** 你会看到 "L 对 c 的梯度" 这样的说法。它回答一个问题：**如果 c 变化一丁点，L 会变化多少？** 比如 "L 对 c 的梯度 = 12" 意思是 "c 增加 1，L 大约增加 12"。

**现在从 L 往回推（就是反向传播）：**

```text
第 1 步：L = c * c，所以 L 对 c 的变化率是多少？
  c * c 对 c 的变化率 = 2*c = 2*6 = 12
  意思是：c 增加 1，L 大约增加 12

第 2 步：c = a * b，所以 c 对 a 的变化率是多少？
  a*b 对 a 的变化率 = b = 3
  意思是：a 增加 1，c 增加 3（因为 b=3）

第 3 步：c = a * b，所以 c 对 b 的变化率是多少？
  a*b 对 b 的变化率 = a = 2
  意思是：b 增加 1，c 增加 2（因为 a=2）
```

**链式法则：把变化率乘起来就行**

```text
L 对 a 的梯度 = (L对c的变化率) * (c对a的变化率) = 12 * 3 = 36
L 对 b 的梯度 = (L对c的变化率) * (c对b的变化率) = 12 * 2 = 24
```

**验证：** a 从 2 变成 3，c = 3*3 = 9，L = 81。变化量 = 81-36 = 45，接近梯度 36（不完全等是因为梯度是"瞬间变化率"，大步走有误差）。

这就是链式法则在做的事：**先看终点怎么变，再一层层把变化量传回去。**
